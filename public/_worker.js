...
const handleUpload = async (request, env, credentials) => {
  ...
  const signedResponse = await fetch(`${getPlatform(env)}/files/generate-upload-url`, {
    method: "POST",
...
const handleGenerate = async (request, env, credentials) => {
  if (request.method !== "POST") return json({ error: "Method not allowed" }, 405);
  let body;
  try {
  ...
  let endpoint;
  try {
    endpoint = getEndpoint(model, env);
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : "Model endpoint missing." }, 503);
  }
  const response = await fetch(`${getPlatform(env)}/${endpoint}`, {
    method: "POST",
    headers: higgsfieldHeaders(credentials, { "Content-Type": "application/json" }),
...
const handleStatus = async (request, env, credentials, requestId) => {
  if (request.method !== "GET") return json({ error: "Method not allowed" }, 405);
  if (!/^[a-zA-Z0-9_-]{8,128}$/.test(requestId || "")) return json({ error: "Invalid request ID." }, 400);
  const response = await fetch(`${getPlatform(env)}/requests/${encodeURIComponent(requestId)}/status`, {
    headers: higgsfieldHeaders(credentials),
  });
  return readUpstream(response);
};

const handleHiggsfield = async (request, env, pathname) => {
  ...
  const path = pathname.slice("/api/higgsfield/".length).split("/").filter(Boolean);
  try {
    if (path[0] === "upload" && path.length === 1) return await handleUpload(request, env, credentials);
    if (path[0] === "generate" && path.length === 1) return await handleGenerate(request, env, credentials);
    if (path[0] === "status" && path[1] && path.length === 2) return await handleStatus(request, env, credentials, path[1]);
...
export default {
  async fetch(request, env) {
    ...
    if (pathname === "/api/higgsfield" || pathname.startsWith("/api/higgsfield/")) {
      return handleHiggsfield(request, env, pathname);
    }
